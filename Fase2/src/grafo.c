...
Listagem *DepthFirstSearch(GrafoAntenas *grafo, int idOrigem, CodigoErro *codigo)
{
    ...
    while (stack != NULL)
    {
        ...
        if (!novo)
        {
            *codigo = ERRO_MEMORIA;
            free(stack);
            return lista; // Devolve o que tiver conseguido até aqui
        }
        ...
        else
        {
            ...
            temp->next = novo; // Adiciona no fim
        }
...
// ResultadoPesquisa *BreadthFirstSearch(GrafoAntenas *grafo, int idOrigem, CodigoErro *codigo)
// {
//     if (!grafo || !grafo->head)
//     {
//         *codigo = ERRO_GRAFO_VAZIO;
...
//     Vertice **queue = calloc(numVertices, sizeof(Vertice *));
//     if (!queue)
//     {
//         free(visitado);
//         *codigo = ERRO_MEMORIA;
